...
Процедура ИзвлечьРедактируемыйЗапрос(ЗапросКроме = Неопределено, ВосстановитьПозийиюРедактирования = Истина)
	...
	ТекстЗапроса = стДанныеЗапроса.Запрос;
...
Процедура ИзменитьИмяПараметраВТекстеЗапроса(РезультатВопроса, ДополнительныеПараметры) Экспорт
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ТекстЗапроса = ЗаменитьПараметр(ТекстЗапроса, "&" + ДополнительныеПараметры.СтароеЗначениеИмяПараметра, "&"
	...
	Если РезультатЗакрытия <> Неопределено Тогда
		...
		ИначеЕсли ДополнительныеПараметры.Поле = "ТипЗначения" Тогда
			...
			Если РезультатЗакрытия.Свойство("ТекстЗапроса", стрТекстЗапроса) Тогда
				...
				чРазмерТекста = СтрДлина(ТекстЗапроса);
...
Функция ЗаполнитьИзXMLНаСервере()
	...
	стрТекстВОкнеЗапроса = ТекстЗапроса;
...
Процедура Команда_СинтаксическийКонтрольЗапроса(Команда)
	...
	Результат = ФорматироватьТекстЗапросаНаСервере(ТекстЗапроса);
...
Процедура Команда_ФорматироватьТекстЗапроса(Команда)
	...
	ТекстКоличество = Новый ТекстовыйДокумент;
	ТекстКоличество.УстановитьТекст(ТекстЗапроса);
	Элементы.ТекстЗапроса.УстановитьГраницыВыделения(1, 1, ТекстКоличество.КоличествоСтрок() + 1, 1);
...
Процедура Команда_ВставитьПредопределенноеЗначение(Команда)
	...
	ПараметрыОткрытия = Новый Структура("Объект, ДанныеФормы, ТекстЗапроса, НачальнаяСтрока, НачальнаяКолонка, КонечнаяСтрока, КонечнаяКолонка",
		Объект, ДанныеФормыВыборПредопределенного, ТекстЗапроса, НачальнаяСтрока, НачальнаяКолонка, КонечнаяСтрока,
		КонечнаяКолонка);
...
Процедура Команда_ПолучитьКодСПараметрами(Команда)
	...
	ПараметрыОткрытия = Новый Структура("
										...
										|ПараметрыЗапроса,
										|Заголовок,
										|Содержание", Объект, ИмяЗапроса, ТекстЗапроса,
...
&НаКлиенте
Процедура УстановитьТекстАлгоритма(НовыйТекст)
