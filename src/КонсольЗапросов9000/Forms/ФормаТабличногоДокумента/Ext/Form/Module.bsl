...
Процедура СформироватьДокумент()
	...
	маРезультатЗапроса = ПолучитьИзВременногоХранилища(Параметры.АдресРезультатаЗапроса);
	...
	Для й = 1 по рзВыборка.Колонки.Количество() Цикл
        ...
        ОбластьЗаполнения.Заполнение = ТипЗаполненияОбластиТабличногоДокумента.Параметр;
		ОбластьЗаполнения.Примечание = Колонка.ТипЗначения;
	...
	ИначеЕсли Параметры.ВидРезультата = "дерево" Тогда
		...
		ВывестиВетку(Документ, СтрокаВывода, СтрокаВыводаГруппировка1, СтрокаВыводаГруппировка1, выбВыборка, тзРазмерыДанных, Обработка, фЕстьМакроколонки, стМакроколонки);
