...
Функция ПолучитьПланЗапросаИзЖурнала()
	...
	Для Каждого Событие Из маСобытия Цикл
		...
		Если ПланТекст.КоличествоСтрок() > 0 Тогда
			ПланТекст.ДобавитьСтроку(РазделительПланов);
			ПланТекст1С.ДобавитьСтроку(РазделительПланов);
		КонецЕсли;
		...
		Иначе
			стрОшибка = СтрШаблон("Получение плана запроса для СУБД ""%1"" не поддерживается.", Событие.DBMS);
			ПланТекст.УстановитьТекст(стрОшибка);
			ПланТекст1С.УстановитьТекст(стрОшибка);
		КонецЕсли;
	...
	//
	//Если Событие.DBMS = "DBMSSQL" Тогда
	//	УстановитьПланЗапроса_DBMSSQL(ДанныеТерминов);
	//ИначеЕсли Событие.DBMS = "DBPOSTGRS" Тогда
	//	УстановитьПланЗапроса_DBPOSTGRS(ДанныеТерминов);
	//Иначе
	//	стрОшибка = СтрШаблон("Получение плана запроса для СУБД ""%1"" не поддерживается.", Событие.DBMS);
	//	ПланТекст.УстановитьТекст(стрОшибка);
	//	ПланТекст1С.УстановитьТекст(стрОшибка);
...
Процедура ДобавитьТекстЗапроса(ДанныеТерминов)
	...
	Если Событие.Свойство("Sql", стрТекст) Тогда
		стрТекст = УбратьКавычки(стрТекст);
		ТекстЗапроса = ДобавитьРазделительЗапросовЕслиНеПустой(ТекстЗапроса) + стрТекст;
		ТекстЗапросаВТерминах1С = ДобавитьРазделительЗапросовЕслиНеПустой(ТекстЗапросаВТерминах1С) + Обработка.SQLЗапросВТермины1С(стрТекст, ДанныеТерминов);
	КонецЕсли;
	
	Если Событие.Свойство("Prm", стрПараметры) Тогда
		ПараметрыЗапроса = ДобавитьРазделительЗапросовЕслиНеПустой(ПараметрыЗапроса) + УбратьКавычки(стрПараметры);
...
Процедура ДобавитьПланЗапроса_DBPOSTGRS(ДанныеТерминов)
	...
	ПланТекст.УстановитьТекст(стрТекстПлана);
	ПланТекст1С.УстановитьТекст(Обработка.SQLПланВТермины1С(стрТекстПлана, ДанныеТерминов, 1));
