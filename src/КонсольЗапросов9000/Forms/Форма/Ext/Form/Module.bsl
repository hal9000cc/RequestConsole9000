...
Функция ИмяКорректно(стрИмя)
	...
	Если НЕ ЗначениеЗаполнено(стрИмя) Тогда
		Возврат Ложь;
	КонецЕсли;
	...
	Для Каждого ЗапросСхемы Из СхемаЗапроса.ПакетЗапросов Цикл
		...
		Если ТипЗнч(ЗапросСхемы) = Тип("ЗапросВыбораСхемыЗапроса") Тогда
			...
			Если ТехнологическийЖурналВключенИРаботает Тогда
				...
				зЗапрос.Текст = СтрШаблон("ВЫБРАТЬ ""%2_begin"" ПОМЕСТИТЬ %2_begin; %1; ВЫБРАТЬ ""%2_end"" ПОМЕСТИТЬ %2_end", ЗапросСхемы.ПолучитьТекстЗапроса(), ЗапросИД);
			Иначе
			...
			маРезультатЗапроса = зЗапрос.ВыполнитьПакет(); //6345bb7034de4ad1b14249d2d7ac26dd
...
Процедура ПакетЗапросов_Сохранить(стрИмяФайла, фТолькоЗаголовок = Ложь)
	...
	стрДанныеДляЗаписи = ПакетЗапросов_СохранитьНаСервере(стДанныеДляЗаписи);
	
 	ДокументЗапись = Новый ТекстовыйДокумент;
	ДокументЗапись.УстановитьТекст(стрДанныеДляЗаписи);
	ДокументЗапись.Записать(стрИмяФайла);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Обход разницы в поведении обычных клиентов и веб.
	//Если Доступность выключить в веб-клиенте при создании на сервере, она работает по другому, чем для обычных клиентов.
...
Процедура ПослеЗавершенияПодключенияРасширенияРаботыСФайлами(Подключено, ДополнительныеПараметры) Экспорт
	...
	Если НЕ Подключено Тогда
		ПоказатьПредупреждение( , "Для работы консоли необходимо установить расширение работы с файлами.", , Объект.Заголовок);
		Закрыть();
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьИзменениеИмениПараметра(НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если НЕ ИмяКорректно(стрИмяПараметра) Тогда
		ПоказатьПредупреждение(, "Неверное имя параметра! Имя должно состоять из одного слова, начинаться с буквы и не содержать специальных символов кроме ""_"".", , Объект.Заголовок);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если маСтрокиИмени.Количество() > 1 Тогда
		ПоказатьПредупреждение(, "Параметр с таким именем уже есть! Введите другое имя.", , Объект.Заголовок);
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ПакетЗапросовСохранитьКак_ПослеВыбораФайла(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Файл = Новый Файл(стрИмяФайла);
	Если ВРег(Файл.Расширение) <> "." + ВРег(стрРасширениеФайлов) Тогда
		Сообщить(Файл.Расширение);
		стрИмяФайла = СтрШаблон("%1.%2", стрИмяФайла, стрРасширениеФайлов);
	КонецЕсли;
...
Функция ПолучитьКонструкторЗапроса(стрТекстЗапроса)
	...
	Исключение
		...
		ПоказатьПредупреждение(, стрОшибка, , Объект.Заголовок);
...
Процедура ВыполнитьЗапрос(фИспользоватьВыделение)
	...
	Если ЗначениеЗаполнено(стРезультат.ОписаниеОшибки) Тогда
		ПоказатьПредупреждение(, стРезультат.ОписаниеОшибки, , Объект.Заголовок);
		ТекущийЭлемент = Элементы.ТекстЗапроса;
...
Процедура Команда_ПараметрыЗаполнитьИзЗапроса(Команда)
	...
	Если ЗначениеЗаполнено(стОшибка) Тогда
		...
		ПоказатьПредупреждение(, стОшибка.ОписаниеОшибки, , Объект.Заголовок);
		ТекущийЭлемент = Элементы.ТекстЗапроса;
...
Процедура Команда_СинтаксическийКонтрольЗапроса(Команда)
	...
	Если ТипЗнч(Результат) <> Тип("Строка") Тогда
		ПоказатьПредупреждение(, Результат.ОписаниеОшибки, , Объект.Заголовок);
		ТекущийЭлемент = Элементы.ТекстЗапроса;
...
Процедура Команда_ДобавитьПерехваченныеЗапросы(Команда)
	...
	ПоказатьПредупреждение(, "Загруженно перехваченных запросов: " + маФайлыЗапросов.Количество(), , Объект.Заголовок);
...
Процедура Команда_УдалитьПерехваченныеЗапросы(Команда)
	...
	УдалитьФайлыНаСервере(маФайлыЗапросов);
	ПоказатьПредупреждение(, "Удалено перехваченных запросов: " + маФайлыЗапросов.Количество(), , Объект.Заголовок);
...
Процедура Команда_РезультатВТабличныйДокумент(Команда)
	...
	ФормаТабличногоДокумента = ОткрытьФорму("ВнешняяОбработка.КонсольЗапросов9000.Форма.ФормаТабличногоДокумента", ПараметрыОткрытия, ЭтаФорма, Ложь);
...
Процедура ОтобразитьСостояниеФоновогоЗадания() Экспорт
	...
	ИначеЕсли СостояниеЗадания.Состояние = 2 Тогда
		ПоказатьПредупреждение(, СостояниеЗадания.ИнформацияОбОшибке, , Объект.Заголовок);
...
Процедура Команда_ВыполнитьОбработку(Команда)
	...
	Если НЕ ЗначениеЗаполнено(РезультатВПакете) ИЛИ Число(РезультатВПакете) <= 0 Тогда
		ПоказатьПредупреждение(, "Выполнение невозможно - результат запроса отсутствует", , Объект.Заголовок);
		Возврат;
	КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ФоновоеЗаданиеИдентификатор) Тогда
		...
		ПоказатьПредупреждение(, "Выполнение прервано пользователем!", , Объект.Заголовок);
		Возврат;
	КонецЕсли;
...
Процедура Команда_ПланЗапроса(Команда)
	...
	Форма = ОткрытьФорму("ВнешняяОбработка.КонсольЗапросов9000.Форма.ФормаПланаЗапроса", ПараметрыОткрытия, ЭтаФорма, Ложь, , , , РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВыполнитьПродолжение(ДополнительныеПараметры)
	...
	//Сообщить(ДополнительныеПараметры.Продолжение);
...
стрРасширениеФайлов = "q9";
стрСигнатураКонсоли = "КонсольЗапросов9000";
стрФильтрСохранения = "Файл запросов (*." + стрРасширениеФайлов + ")|*." + стрРасширениеФайлов;
