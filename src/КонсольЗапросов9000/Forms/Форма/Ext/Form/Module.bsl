...
Процедура ПриОткрытииЗавершение()
	...
	//Почему-то чудный релиз 8.3.17 без режима совместимости не устанавливает изначально текущую строку.
...
Функция ТехнологическийЖурнал_Включился()
	Возврат РеквизитФормыВЗначение("Объект").ТехнологическийЖурнал_Включился();
КонецФункции

&НаСервере
Функция ТехнологическийЖурнал_Выключился()
	Возврат РеквизитФормыВЗначение("Объект").ТехнологическийЖурнал_Выключился();
КонецФункции
...
Процедура ТехнологическийЖурнал_ОжиданиеВключения() Экспорт
	...
	Иначе
		Если ТекущаяУниверсальнаяДатаВМиллисекундах() - ТехнологическийЖурналВремяВключенияВыключения < 65 * 1000 Тогда
...
Процедура ТехнологическийЖурнал_ОжиданиеВыключения() Экспорт
	...
	Иначе
		...
		Иначе
			...
			//Но конфиг исправлен, прошло 65 секунд. Будем считать, что он выключен, других вариантов нет.
			ТехнологическийЖурнал_ИндикацияВключения(Ложь);
		КонецЕсли;
	КонецЕсли;
