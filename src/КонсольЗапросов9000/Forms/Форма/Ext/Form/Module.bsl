...
Процедура ПакетЗапросов_Сохранить(стрИмяФайла, фТолькоЗаголовок = Ложь)
	...
	стДанныеДляЗаписи = Новый Структура("
		...
		КоличествоЗапросов,
		ИмяФайлаЗапросов,
		ПолучитьИзВременногоХранилища(Объект.АдресСохраняемыеСостояния),
		стОпции);
...
Функция ПакетЗапросов_Загрузить(стрИмяФайла)
	...
	Если НЕ ЗначениеЗаполнено(Объект.АдресСохраняемыеСостояния) Тогда
		Если стЗагруженныеДанные.Версия >= 11 Тогда
			Объект.АдресСохраняемыеСостояния = ПоместитьВоВременноеХранилище(стЗагруженныеДанные.СохраняемыеСостояния, УникальныйИдентификатор);
		Иначе
			Объект.АдресСохраняемыеСостояния = ПоместитьВоВременноеХранилище(Новый Структура, УникальныйИдентификатор);
		КонецЕсли;
	КонецЕсли;
	...
	//Если стЗагруженныеДанные.Версия >= 11 И НЕ ЗначениеЗаполнено(Объект.АдресСохраняемыеСостояния) Тогда
	//	Объект.АдресСохраняемыеСостояния = ПоместитьВоВременноеХранилище(стЗагруженныеДанные.СохраняемыеСостояния, УникальныйИдентификатор);
	//Иначе
	//	Объект.АдресСохраняемыеСостояния = ПоместитьВоВременноеХранилище(Новый Структура, УникальныйИдентификатор);
	//КонецЕсли;
...
&НаКлиенте
Процедура Команда_ПанельВременныхТаблиц(Команда)
