...
Процедура ПриОткрытииЗавершение()
	...
	Если Элементы.ТехнологическийЖурнал.Пометка Тогда
		...
		ПодключитьОбработчикОжидания("ТехнологическийЖурнал_ОжиданиеВключения", 0.1, Истина);
	КонецЕсли;
...
Процедура Команда_ПланЗапроса(Команда)
	...
	ПараметрыОткрытия = Новый Структура("Объект, АдресРезультатаЗапроса, РезультатВПакете", Объект, АдресРезультатаЗапроса, ПакетРезультатаЗапроса.Индекс(ПакетРезультатаЗапроса.НайтиПоИдентификатору(ТекущаяСтрока)) + 1);
	...
	Если Форма = Неопределено Тогда
		ПоказатьПредупреждение(, "Не удалось получить информацию о запросе", , Заголовок);
	КонецЕсли;
