...
Функция ПолучитьЛитералЗначения(Значение)
	...
	ИначеЕсли Перечисления.ТипВсеСсылки().СодержитТип(ТипЗначения) Тогда
		...
		стрЛитерал = СтрШаблон("Перечисление.%1.%2", Значение.Метаданные().Имя, ИдентификаторПеречисления);
	...
	Если стрЛитерал = Неопределено Тогда
		Возврат Новый Структура("Литерал, Комментарий", "???");
	КонецЕсли;
	
	Возврат Новый Структура("Литерал, Комментарий", стрЛитерал, стрКомментарий);
...
Функция ПолучитьКодСПараметрами(ИмяЗапроса)
	...
	Для Каждого СтрокаПараметра Из ПараметрыЗапроса Цикл
		...
		стЛитерал = ПолучитьЛитералЗначения(Значение);
		
		стрПараметр = СтрШаблон("	%1.УстановитьПараметр(""%2"", %3);", ИмяЗапроса, СтрокаПараметра.Имя, стЛитерал.Литерал);
