...
Функция ИзвлечьРезультатКакТаблицуЗначений()
	...
	маРезультатЗапроса = ПолучитьИзВременногоХранилища(АдресРезультатаЗапроса);
...
Функция ВыполнитьЗапросНаСервере(стрТекстЗапроса)
	...
	Если ЗначениеЗаполнено(АдресРезультатаЗапроса) Тогда
		АдресРезультатаЗапроса = ПоместитьВоВременноеХранилище(маРезультатЗапроса, АдресРезультатаЗапроса);
	Иначе
		АдресРезультатаЗапроса = ПоместитьВоВременноеХранилище(маРезультатЗапроса, УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ВыполнитьЗапрос(фИспользоватьВыделение)
	...
	стОшибка = ВыполнитьЗапросНаСервере(стрТекстЗапроса);
	Если ЗначениеЗаполнено(стОшибка) Тогда
		...
		Если ЗначениеЗаполнено(стОшибка.Строка) Тогда
			...
			Иначе
				Если стОшибка.Строка = 1 Тогда
					чСтрока = НачалоСтроки;
					чКолонка = НачалоКолонки + стОшибка.Колонка - 1;
				КонецЕсли;
				Элементы.ТекстЗапроса.УстановитьГраницыВыделения(чСтрока, чКолонка, чСтрока, чКолонка);
			КонецЕсли;
		КонецЕсли;
	Иначе
		...
		Элементы.ПакетЗапросов.ТекущиеДанные.Инфо = Строка(ТекущаяСтрокаПакета.КоличествоВозвращенныхСтрокРезультата) + " / " + Формат('00010101' + (ТекущаяДата() - ВремяНачала), "ДЛФ=T; ДП=0:00:00");
...
&НаСервереБезКонтекста
Функция Код_ВыполнитьОбработкуПрогресс(Выборка, ЭтотКод, МетодИсполненияКода, АдресХраненияСостояния)
		
	Попытка
		
		фПродолжить = Ложь;
		Пока Выборка.Следующий() Цикл
			ВыполнитьКод(Выборка, ЭтотКод);
			фПродолжить = Истина;
			Прервать;
		КонецЦикла;
		
		стРезультат = Новый Структура("Успешно, Продолжить, ОписаниеОшибки", Истина, фПродолжить, Неопределено);
		
	Исключение
		
		стрОписаниеОшибки = ОписаниеОшибки();
		стРезультат = Новый Структура("Успешно, Продолжить, ОписаниеОшибки", Ложь, Ложь, стрОписаниеОшибки);
...
Функция ЗапуститьОбработкуНаСервере(фПострочно = Истина)
	...
	ПараметрыВыполнения.Добавить(маРезультатЗапроса);
...
Функция ПолучитьСостояниеФоновогоЗадания()
	...
	Если ФоновоеЗадание.ИнформацияОбОшибке <> Неопределено Тогда
		СостояниеЗадания.ИнформацияОбОшибке = ФоновоеЗадание.ИнформацияОбОшибке.Описание + ":
		|" + ФоновоеЗадание.ИнформацияОбОшибке.ИсходнаяСтрока;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция ВыполнитьКод(Выборка, ЭтотКод)
	Выполнить(ЭтотКод);	
КонецФункции
...
Функция ВыполнитьАлгоритм()
	...
	Попытка
		ВыполнитьКод(выбВыборка, ТекстАлгоритм.ПолучитьТекст());
	Исключение
...
Функция ВыполнитьАлгоритмПострочно(АдресРезультатаЗапроса, РезультатВПакете, ТекстАлгоритма)
	...
	Попытка
		Пока выбВыборка.Следующий() Цикл
			ВыполнитьКод(выбВыборка, ТекстАлгоритма);
		КонецЦикла;
	Исключение
		стрСообщениеОбОшибке = ОписаниеОшибки();
		Возврат Новый Структура("Успешно, Продолжать, ОписаниеОшибки", Ложь, Ложь, стрСообщениеОбОшибке);
...
Функция ВыполнитьАлгоритмНаСервереПострочно(АдресСостояния, АдресРезультатаЗапроса, РезультатВПакете, ТекстАлгоритма, ОпцияИнтервалОбновленияВыполненияАлгоритма)
	...
	Если стСостояние = Неопределено Тогда
		...
		рзВыборка = стРезультат.Результат;
		выбВыборка = рзВыборка.Выбрать();
		стСостояние = Новый Структура("Выборка, КоличествоВсего, КоличествоСделано, Начало, НачалоВМиллисекундах", выбВыборка, выбВыборка.Количество(), 0, ТекущаяДата(), ТекущаяУниверсальнаяДатаВМиллисекундах());
	КонецЕсли;
...
Процедура Команда_РезультатВПараметр(Команда)
	...
	ОписаниеОповещенияОЗакрытииОткрываемойФормы = Новый ОписаниеОповещения("ОкончаниеРедактированияСтроки", ЭтаФорма, ПараметрыОповещения);
	ПараметрыОткрытия = Новый Структура("Объект, ТипЗначения, ТипКонтейнера, Имя, ВЗапросРазрешено", Объект, тзТаблица, 3, ИмяЗапросаРезультата, Истина);
	ОткрытьФорму("ВнешняяОбработка.КонсольЗапросов9000.Форма.РедактированиеТипа", ПараметрыОткрытия, ЭтаФорма, Истина, , , ОписаниеОповещенияОЗакрытииОткрываемойФормы, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаСервере
Функция ПолучитьЛитералЗначения(Значение)
	...
	ИначеЕсли ТипЗначения = Тип("Дата") Тогда
		...
		стрЗначение = Формат(Г, "ЧЦ=4; ЧГ=") + Формат(М, "ЧЦ=2; ЧГ=") + Формат(Д, "ЧЦ=2; ЧГ=");
		Если Ч <> 0 ИЛИ Мин <> 0 И Сек <> 0 Тогда
			стрЗначение = стрЗначение + Формат(Ч, "ЧЦ=2; ЧГ=") + Формат(Мин, "ЧЦ=2; ЧГ=") + Формат(Сек, "ЧЦ=2; ЧГ=");
		КонецЕсли;
	...
	ИначеЕсли ТипЗначения = Тип("УникальныйИдентификатор") Тогда
		Возврат СтрШаблон("Новый УникальныйИдентификатор(""%1"")", Строка(Значение));
...
Функция ПолучитьКодСПараметрами(ИмяЗапроса)
	...
	Текст.ЗаменитьСтроку(Текст.ПолучитьСтроку(Текст.КоличествоСтрок()) + """);");
	
	Для Каждого СтрокаПараметра Из ПараметрыЗапроса Цикл
		Значение = ПараметрыЗапроса_ПолучитьЗначение(СтрокаПараметра.ПолучитьИдентификатор());
		стрЗначение = ПолучитьЛитералЗначения(Значение);
		Текст.ДобавитьСтроку("	%1.УстановитьПараметр(%2, %3);", ИмяЗапроса, СтрокаПараметра.Имя, стрЗначение);
	КонецЦикла;
