...
Функция ВыполнитьКод(Выборка, ЭтотКод, ПризнакПрогресса)
	Выполнить(ЭтотКод);	
КонецФункции
...
Процедура ВыполнитьАлгоритмПользователя(ПараметрыВыполнения, АдресРезультата) Экспорт
	...
	маРезультатЗапроса = ПараметрыВыполнения[0];
	...
	Если фПострочно Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ВыполнитьКод(Выборка, Код, АдресРезультата);
