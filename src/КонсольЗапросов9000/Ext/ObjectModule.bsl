...
Процедура Инициализация(Форма = Неопределено, чСеансИД = Неопределено) Экспорт
	...
	Если Сред(ИспользуемоеИмяФайла, 2, 2) = ":\" ИЛИ Лев(ИспользуемоеИмяФайла, 1) = "/" Тогда
...
Функция ТехнологическийЖурнал_ПолучитьФрагментЖурналаПоИдентификаторуИВремени(Идентификатор, ВремяНачалаЗапроса, ВремяОкончанияЗапроса) Экспорт
	...
	Для Каждого Лог Из маЛоги Цикл
		...
		Пока НЕ фФрагментНайден И ВремяПоиска < ВремяОкончанияЗапроса Цикл
			...
			стрИмяФайлаЛога = Формат(ВремяПоиска, "ДФ=ггММддЧЧ.log");
			ВремяПоиска = ВремяПоиска + 60 * 60;
			стрПолноеИмяФайлаЛога = СтрШаблон("%1%2%3", Лог.ПолноеИмя, ПолучитьРазделительПутиСервера(), стрИмяФайлаЛога);
			
			Файл = Новый Файл(стрПолноеИмяФайлаЛога);
			Если НЕ Файл.Существует() Тогда
				Продолжить;
			КонецЕсли;
			
			Чтение = Новый ЧтениеДанных(стрПолноеИмяФайлаЛога);
...
Функция ТехнологическийЖурнал_ПолучитьИнформациюПоЗапросу(Идентификатор, ВремяНачалаЗапроса, ДлительностьЗапроса) Экспорт
	...
	стрФрагментЖурнала = ТехнологическийЖурнал_ПолучитьФрагментЖурналаПоИдентификаторуИВремени(Идентификатор, ВремяПоискаНачало, ВремяПоискаКонец);
	
	Возврат стрФрагментЖурнала;
...
Функция SQLПланВТермины1С(ПланЗапроса, ДанныеТерминов) Экспорт
...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	ПараметрыРегистрации.Вставить("Версия", ВерсияОбработки + ".13");
